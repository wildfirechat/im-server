...
3. 从0.53之前版本升级到0.53及以后版本时，会提示flyway migrate错误。因为0.53版本删除了sql脚本中默认敏感词的内容，flyway checksum失败。请执行```update flyway_schema_history set checksum = 0 where script = 'V17__add_default_sensitive_word.sql';```来修复。
